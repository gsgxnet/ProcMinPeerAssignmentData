...
1. Loading the file Receipt phase of an environmental permit application process (_WABO_) CoSeLoG project.xes.gz into Prom67 and visualizing it by a Dotted Chart diagram and while sorting by time:startTime of Trace gives an overview of case start rate.  
...
- there is a genral trend for a slightly lower rate of new cases from around 20th of August
...
The essential step to discover a comprehensible Petri net from the supplied log data is to filter out classes of traces with only a few class member but which are complicating the process model without giving extra insights. The major challange is setting the filters to such a level that an optimum of the real process variants is captured. Whether a class of traces is included or not for a good process modell should not depend on miner choosen for a Petri net. But there are differences in features a process discovery method is able to guarantee. Book chaper 7.5 "Inductive Mining" states "process trees are sound by construction".  
...
2. ![](./screenshots/E-1iVM0prefilter.png)  
...
1. Having 116 trace variants and about 75 percent of these being followed only once suggests many very individual case handlings not really following a normative model. Reasons for this should be investigated. A more streamlined handling my be a lot more effective.  
